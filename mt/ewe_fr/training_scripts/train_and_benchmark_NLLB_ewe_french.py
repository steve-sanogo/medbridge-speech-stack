...
# Dictionnaire des chemins de fichiers pré-existants
files = {
...
def create_datasets(debug=False):
    """
    Charge les datasets CSV préalablement filtrés et splittés.
    """
    print(f"📂 Loading Pre-processed Datasets from {SAVE_DIR_SPLIT}...")
    
    # Vérification de l'existence des fichiers
    for key, path in files.items():
        if not path.exists():
            raise FileNotFoundError(f"❌ Le fichier {key} est introuvable à l'endroit : {path}. "
                                    f"Lance d'abord le script de filtrage sémantique.")
    ...
    if debug:
        print("🐞 Debug mode: reducing data size to 100 samples.")
...
def train_model(train_df, val_df, test_df):
    ...
    # On ajoute load_from_cache_file=False pour forcer HF à recalculer et ne pas lire un vieux cache corrompu
...
if __name__ == "__main__":
    ...
    print("*"*50)
    print(f"Script : train_and_benchmark_NLLB_ewe_french.py")
    print("*"*50)
    print(f"✅ Starting Ewe-French Training with only Organic Pairs")
