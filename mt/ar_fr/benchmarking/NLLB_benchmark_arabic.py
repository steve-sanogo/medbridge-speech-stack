...
DATA_DIR     = BASE / "data" / "data_arab" / "NileTTS"
RESULTS_DIR  = BASE / "experiments" / "backtranslation_arabic"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--filename",    type=str, default="metadata_eval.csv", help="Nom du fichier pipe-separated dans DATA_DIR")
    return p.parse_args()
...
def main() -> None:
    ...
    # ── 1. Chargement des données (NileTTS structure) ───────────────────────────────────────────────
    print(f"\n[2/4] Chargement des données ({args.filename})...")
    
    metadata_path = DATA_DIR / args.filename
    if not metadata_path.exists():
        raise FileNotFoundError(f"Fichier {args.filename} introuvable dans {DATA_DIR}")

    df = pd.read_csv(metadata_path, sep='|', quoting=3, on_bad_lines='skip')
